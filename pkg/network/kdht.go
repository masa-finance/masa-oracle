...
func WithDht(ctx context.Context, host host.Host, bootstrapNodes []multiaddr.Multiaddr,
	...
	if len(bootstrapNodes) > 0 && peerConnectionCount == 0 {
		...
		// logrus.Fatal("Unable to connect to a boot node at this time. Please try again later.")
		logrus.Info("Unable to connect to a boot node at this time. Please try again later.")
	}
