...
func ScrapeFollowersForProfile(username string, maxUsersNbr int) ([]twitterscraper.Legacy, error) {
	scraper := auth()
	...
	followingResponse, errString, _ := scraper.FetchFollowers(username, maxUsersNbr, "")
	if errString != "" {
